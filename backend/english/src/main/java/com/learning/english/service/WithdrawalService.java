...
public class WithdrawalService {
    ...
    public TeacherWithdrawalSummaryResponse getMySummary() {
        ...
        return TeacherWithdrawalSummaryResponse.builder()
    ...
    public List<WithdrawalResponse> getMyWithdrawals() {
        User teacher = getCurrentUser();
        return withdrawalRepository.findByTeacher_UserIdOrderByRequestedAtDesc(teacher.getUserId())
                .stream()
    ...
    @Transactional
    public WithdrawalResponse reviewWithdrawal(
            Long withdrawalId,
            String status,
            String rejectReason,
            User admin
        ...
        if ("REJECTED".equalsIgnoreCase(status)) {
            ...
            markEarningsWithdrawn(withdrawal.getTeacher().getUserId(), withdrawal.getAmount());
        ...
        try {
            ...
            if ("PAID".equalsIgnoreCase(withdrawal.getStatus())) {
                ...
                message = String.format(
                        "Yêu cầu rút %s VND đã được admin xác nhận thanh toán.",
                        withdrawal.getAmount().toPlainString()
                );
            } else {
                title = "Yêu cầu rút tiền bị từ chối";
                message = String.format(
                        "Yêu cầu rút %s VND đã bị từ chối. Lý do: %s",
                        withdrawal.getAmount().toPlainString(),
