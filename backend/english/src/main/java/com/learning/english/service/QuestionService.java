...
public class QuestionService {
	...
	public List<QuestionBankItemResponse> layNganHangCauHoiCuaGiaoVien(String questionType, String keyword,
			Long levelId) {
		...
		return questionRepository.findMyQuestionBankByType(username, questionType, keyword, levelId).stream()
				.map(questionMapper::toQuestionBankItemResponse).toList();
	}

	public Page<QuestionResponse> layNganHangCauHoiCuaGiaoVien(String questionType, String keyword,
			Long levelId, int page, int size) {
		String username = getCurrentUsername();
		...
		return questionRepository.findMyQuestionBank(username, questionType, keyword, levelId, pageable)
				.map(questionMapper::toQuestionResponse);
	}

	@Transactional
	public QuestionResponse taoCauHoiVaGanVaoLesson(Long lessonId, QuestionRequest request, MultipartFile mediaFile)
			throws IOException {
	...
	@Transactional
	public List<QuestionResponse> taoNhieuCauHoiVaGanVaoLesson(Long lessonId, List<QuestionRequest> requests) {
		...
		for (int i = 0; i < requests.size(); i++) {
			...
			try {
				...
				/*
				 * Import Excel hiện không có audio file. Nếu request có mediaUrl thì vẫn cho
				 * lưu mediaUrl.
				 */
	...
	@Transactional
	public List<QuestionResponse> taoNhieuCauHoiVaoNganHang(List<QuestionRequest> requests) {
		if (requests == null || requests.isEmpty()) {
			throw new RuntimeException("Danh sách câu hỏi không được để trống");
		}
	...
	@Transactional
	public QuestionResponse themCauHoiVaoNganHang(QuestionRequest request, MultipartFile mediaFile) throws IOException {
	...
	private Question buildQuestionFromRequest(QuestionRequest request, User teacher, String mediaUrl,
		...
		if (isChoiceType(request.getQuestionType())) {
			...
			for (QuestionOptionRequest optionRequest : request.getOptions()) {
				...
				QuestionOption option = QuestionOption.builder().optionText(optionRequest.getOptionText().trim())
						.isCorrect(Boolean.TRUE.equals(optionRequest.getIsCorrect())).createdAt(now).build();
	...
	@Transactional
	public List<QuestionResponse> ganCauHoiCuVaoLesson(Long lessonId, QuestionAttachRequest request) {
		String username = getCurrentUsername();
		...
		List<Question> questions = questionRepository.findMyQuestionsByIdsAndType(username, request.getQuestionType(),
	...
	private void ganQuestionVaoLessonNeuChuaGan(Lesson lesson, Question question) {
		boolean exists = lessonQuestionRepository.existsByLesson_LessonIdAndQuestion_QuestionId(lesson.getLessonId(),
		...
		LessonQuestion lessonQuestion = LessonQuestion.builder().lesson(lesson).question(question).build();

		lessonQuestionRepository.save(lessonQuestion);
	}

	private void ensurePracticeConfigExists(Lesson lesson, String practiceType) {
		boolean exists = practiceConfigRepository.existsByLesson_LessonIdAndPracticeType(lesson.getLessonId(),
				practiceType);
		...
		LessonPracticeConfig config = LessonPracticeConfig.builder().lesson(lesson).practiceType(practiceType)
				.isEnabled(true).createdAt(now).updatedAt(now).build();

		practiceConfigRepository.save(config);
	}

	private void validateCreateRequest(QuestionRequest request, MultipartFile mediaFile) {
		if (request.getContent() == null || request.getContent().isBlank()) {
			throw new RuntimeException("Nội dung câu hỏi không được để trống");
		}

		if (request.getDefaultPoint() != null && request.getDefaultPoint().compareTo(BigDecimal.ZERO) <= 0) {
	...
	private void validateOptions(List<QuestionOptionRequest> options) {
		...
		List<QuestionOptionRequest> validOptions = options.stream()
				.filter(item -> item.getOptionText() != null && !item.getOptionText().isBlank()).toList();

		if (validOptions.size() < 2) {
			throw new RuntimeException("Câu hỏi chọn đáp án cần ít nhất 2 đáp án");
		}

		long correctCount = validOptions.stream().filter(item -> Boolean.TRUE.equals(item.getIsCorrect())).count();
	...
	private void validateQuestionType(String questionType) {
		...
		boolean valid = questionType.equals(MULTIPLE_CHOICE) || questionType.equals(LISTENING_CHOICE)
				|| questionType.equals(LISTENING_FILL_BLANK) || questionType.equals(ARRANGE_SENTENCE)
	...
	private boolean isChoiceType(String questionType) {
		return MULTIPLE_CHOICE.equals(questionType) || LISTENING_CHOICE.equals(questionType);
	}

	private boolean isListeningType(String questionType) {
		return LISTENING_CHOICE.equals(questionType) || LISTENING_FILL_BLANK.equals(questionType);
	}
