...
import com.learning.english.service.SseService;
...
public class SePayWebhookController {
    ...
    public ResponseEntity<SePayWebhookResponse> nhanWebhookSePayChuyenKhoan(
            @RequestBody byte[] rawBody,
            @RequestHeader(value = "X-SePay-Signature", required = false) String signature,
            @RequestHeader(value = "X-SePay-Timestamp", required = false) String timestampHeader
    ) {
        try {
            ...
            if (laHoanTien(request)) {
            	System.out.println("=========hoàn tiền");
                sePayWebhookService.xuLyHoanTienSePay(request);
            } else {
                sePayWebhookService.xuLyChuyenTienSePay(request);
            }
    ...
    private boolean laHoanTien(SePayWebhookRequest request) {
        return coMaHoanTien(request.getCode())
                || coMaHoanTien(request.getContent())
                || coMaHoanTien(request.getDescription());
    }

    private boolean coMaHoanTien(String text) {
        if (text == null || text.isBlank()) return false;

        return text.toUpperCase().matches(".*SEVQRHT\\d+.*");
    }
