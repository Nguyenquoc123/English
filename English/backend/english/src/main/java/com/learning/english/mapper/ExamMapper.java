...
public interface ExamMapper {
	...
	default ExamListResponse toExamListResponse(Object[] row) {
        ...
        return ExamListResponse.builder()
                .examId(toLong(row[0]))
                .courseId(toLong(row[1]))
                .courseTitle(toStringValue(row[2]))
                .title(toStringValue(row[3]))
                .description(toStringValue(row[4]))
                .durationMinutes(toInteger(row[5]))
                .questionCount(toLong(row[6]))
                .status(toStringValue(row[7]))
                .startTime(toLocalDateTime(row[8]))
                .endTime(toLocalDateTime(row[9]))
    ...
    default StudentExamListResponse toStudentExamListResponse(Object[] row) {
        ...
        return StudentExamListResponse.builder()
                ...
                .description(toStringValue(row[4]))
                .durationMinutes(toInteger(row[5]))
                .maxAttempts(toInteger(row[6]))
                .questionCount(toLong(row[7]))
                .totalPoint(toBigDecimal(row[8]))
                .status(toStringValue(row[9]))
                .startTime(toLocalDateTime(row[10]))
                .endTime(toLocalDateTime(row[11]))
    ...
    default List<StudentExamListResponse> toStudentExamListResponses(List<Object[]> rows) {
        ...
        return rows.stream()
                .map(this::toStudentExamListResponse)
                .toList();
    }
