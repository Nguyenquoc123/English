...
import "./ChiTietBaiLam.css";

function ChiTietBaiLam() {
  ...
  const layChiTietLanLamBai = async () => {
    try {
      setLoading(true);
      setError("");
      ...
      const response = await fetch(`${API_BASE}/practice-attempts/${attemptId}/result`, {
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        setError(body?.message || "Không thể tải chi tiết bài làm");
        return;
      }
  ...
  const isExam = result?.attemptType === "EXAM" || result?.examId;
  const isPractice = result?.attemptType === "PRACTICE" || result?.lessonId;

  const getPageTitle = () => {
    if (isExam) return "Kết quả bài thi";
    if (isPractice) return "Kết quả ôn tập";
    return "Chi tiết bài làm";
  };

  const getAttemptName = () => {
    if (isExam) return result?.examTitle || "Bài thi";
    if (isPractice) return result?.lessonTitle || "Bài ôn tập";
    return result?.examTitle || result?.lessonTitle || "Bài làm";
  };
  ...
  const percent = useMemo(() => {
    ...
    return Math.round(((result.totalCorrect || 0) / result.totalQuestions) * 100);
  }, [result]);
  ...
  const getAnswerDisplay = (detail) => {
    if (detail.selectedOptionText) return detail.selectedOptionText;
    if (detail.answerText) return detail.answerText;
    if (detail.studentAnswer) return detail.studentAnswer;
    if (detail.userAnswer) return detail.userAnswer;

    return "Chưa trả lời";
  };

  const getCorrectAnswerDisplay = (detail) => {
    if (detail.correctAnswer) return detail.correctAnswer;
    if (detail.correctOptionText) return detail.correctOptionText;
  ...
  const handleRetry = () => {
    if (isPractice && result?.lessonId && result?.practiceType) {
      navigate(`/khoa-hoc/${result.courseId}/lessons/${result.lessonId}/practice/${result.practiceType}`);
      return;
    }
      ...
      <div className="practice-result-container">
        <div className="result-topbar">
          <div>
            ...
            <h2>{getPageTitle()}</h2>
            <p>Xem tổng quan kết quả và chi tiết từng câu trả lời.</p>
          </div>
        ...
        <div className="result-summary-grid">
          <div className="result-info-card">
            ...
            <div className="result-info-list">
              <div>
                <span>Tên bài</span>
                <strong>{getAttemptName()}</strong>
              </div>

              <div>
                <span>Loại bài</span>
                <strong>{getAttemptTypeName(result.attemptType)}</strong>
              </div>
          ...
          <div className="score-card">
            ...
            <div className="score-icon">
              <i className={isExam ? "bi bi-mortarboard-fill" : "bi bi-trophy-fill"}></i>
            </div>
          </div>
        </div>
          ...
          {result.details && result.details.length > 0 ? (
            <div className="answer-list">
              {result.details.map((detail, index) => (
                <div
                  className={detail.isCorrect ? "answer-item correct" : "answer-item wrong"}
                  key={detail.attemptDetailId || detail.questionId || index}
                >
                  <div className="answer-item-header">
                    ...
                    <div className="question-title-area">
                      ...
                      <h5>{detail.content || detail.questionContent || "Không có nội dung câu hỏi"}</h5>
                    </div>
                  </div>

                  {detail.audioUrl && (
                    <div className="mt-3">
                      <audio controls src={detail.audioUrl} className="w-100">
                  ...
                  <div className="answer-compare-grid">
                    ...
                    <div className="answer-box correct-answer">
                      ...
                      <strong>{getCorrectAnswerDisplay(detail)}</strong>
                    </div>
                  </div>
